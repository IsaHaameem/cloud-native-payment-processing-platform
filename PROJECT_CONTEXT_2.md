...
> **M23.2 complete** (2026-08-09): the portal authenticates. An AES-256-GCM sealed session
> cookie (D196), a refresh coordinator of mutex plus replay cache (D197), `middleware.ts` as the
> single refresh point on the Node runtime (D198), CSRF defended three ways (D199), a per-account
> sign-in throttle (D200), and the `requireSession`/`requireMerchant`/`requireRole` guards. No
> token of any kind reaches the browser, asserted in a real one by `scripts/verify-auth.mjs`.
> **M23.2a complete** (2026-08-09): the *entry* flow, which M23.2 left with no front door — a
> user could sign in and nothing could create the account or the merchant to sign in with. Adds
> `/signup` against `POST /api/v1/auth/register`, a real `/onboarding` against
> `POST /api/v1/merchants`, and `/dashboard` as the authenticated entry point §6.1 fixes; the
> landing page becomes a product surface with a public navbar rather than a milestone
> announcement. **No backend change at all** — every step was already served by an existing
> endpoint, which is what made this a UX gap rather than a missing capability (D201–D203).
> **M23.2b complete** (2026-08-09): two authentication defects found by using the product.
> **(1)** Every state-changing form was refused when the portal was reached at `127.0.0.1` while
> configured for `localhost` — the same server, a spelling a browser offers by itself — because
> the CSRF origin check compared against one configured string. It was reported as a signup bug
> because a cross-origin refusal and a stale CSRF token shared one message. Both halves fixed:
> sibling loopback spellings are accepted when the portal is itself configured for a loopback
> address, an opt-in `PORTAL_ADDITIONAL_ORIGINS` covers multi-origin deployments, and the two
> failures now say different things and log the mismatch (D204, D205).
> **(2)** Password recovery had no UI, though identity-service has served
> `POST /api/v1/auth/password-reset/{request,confirm}` since M15 and has been emailing links to
> `{baseUrl}/reset-password?token=…` at a route that did not exist. `/forgot-password` and
> `/reset-password` now close that loop against the real contract — **no backend change** (D206).
> **M23.3 complete** (2026-08-09): the shell and the data layer. TanStack Query behind a provider
> created per mount (D191's choice, finally instantiated), a key factory in which the **mode and
> the merchant are structurally unavoidable** — the only way to build a key takes the scope as its
> first argument — and one new endpoint, `/api/platform/[operation]`, the single door a client
> component may read the platform through. That endpoint resolves **GETs only**, derived from the
> generated descriptors rather than listed, so no mutation is reachable from the browser and the
> set cannot drift from `docs/openapi.yaml` (D207). Cursor pagination, scope-wide invalidation,
> breadcrumbs, route-change focus management, and the command palette's object lookup — built on
> what the **contract** says ids look like (UUIDs for payments and refunds; only events carry a
> prefix), not the `pay_…` the M23.1 note assumed (D208). Verified against the real local stack: a
> payment created through the real API, resolved in the palette as *"Payment …ba19e6 · created ·
> €42.00"*. **No backend change** (D209).
> **M23.4 complete** (2026-08-09): merchant settings. `/settings` reads the account from
> identity-service and the business from merchant-service, and edits the business half through
> Server Actions — never through M23.3's read route, which resolves GETs only by design. Both
> endpoints are `/me`: the owner comes from the JWT subject, so the portal names no merchant and
> makes no authorization decision beyond holding a session. A save confirms itself through a
> **full document navigation** to a query flag, after client-side alternatives were each measured
> failing in a different way (D210). The screen states the two things §6.1 puts on it that the
> platform cannot answer — team membership does not exist as an entity, and `pinned_api_version`
> is not on `MerchantResponse` — rather than omitting them silently (D211). M23.4's other half,
> onboarding, shipped early in M23.2a because a new account needed some way to reach the dashboard
> at all, and is verified by the public-flow suite. Verified against the real local stack: a
> rename stored by merchant-service and reflected in the shell's header, an `http://` callback URL
> refused, an `https://` one stored and then genuinely cleared. **No backend change**.
> **M23.5 complete** (2026-08-09): API keys — create, reveal-once, rotate, revoke, at
> `/developers/api-keys`. The raw secret exists in readable form for exactly one response and is
> shown behind an explicit copy-and-acknowledge step; the page that lists keys reads
> `ApiKeyResponse`, which carries no secret, so a reload ends the reveal permanently. Verified from
> the outside: the value read off the screen appears in no page source, URL, cookie, storage or
> reload — and it **authenticates the real `/v1` API**, which is what makes it a credential rather
> than a string. Revocation requires typing the key's name, checked against the name
> merchant-service holds rather than against a hidden field (D212). One backend change, the only
> one M23.5 needed: `ApiKeyResponse` gained `graceExpiresAt`, without which a rotated-out key is
> indistinguishable on the wire from the replacement that superseded it (D213). It is account-plane
> only — `PublicApiDocumentContract` asserts `/api/v1` is excluded from the published spec — so
> `docs/openapi.yaml`, the SDKs and the generated trees are untouched. Verified against the real
> local stack: rotation puts the old key into a 24-hour grace window during which it still returns
> 200, and revocation makes it 401 on the next request.
> **M23.6 complete** (2026-08-10): the payments list at `/payments` — the first screen to consume
> M23.3's cursor pagination, which D207 predicted would stay unbuildable until a list screen
> existed. Server-side filtering on status, currency, amount range, date range and metadata; the
> **URL is the view**, since no preferences entity exists anywhere to persist one and a link is
> shareable, bookmarkable and back-button-correct where a private preset is none of those (D215).
> CSV export names its row count, because there is no export endpoint and paging silently to the
> end of a merchant's history behind one click is not what a button that looks instant should do
> (D216). One portal-side change was genuinely required: the transport and the read route now
> accept the **`deepObject` spelling** of a declared map parameter, without which §6.2's metadata
> filter could not be sent at all (D217). Nothing is optimistic and nothing is computed — M23.6
> issues no mutations, so every figure on screen is a field the platform returned. Verified against
> the real local stack with 30 real payments created through `POST /v1/payments`: the rows, their
> order and their count match what payment-service holds, and a live payment is invisible in test
> mode and vice versa. **No backend change, no contract change.**
> **Milestone IDs continue from V1:** V2 begins at **M15**.
> **Decision IDs continue from V1:** V1 ended at **D97**; V2's log now runs **D98–D217**.
...
| D201 | (M23.2a) Registration **does not sign the user in**; `/signup` creates the account and redirects to `/login` with a confirmation and the address pre-filled | (a) Replay the just-typed password into `login` and establish a session immediately; (b) have identity-service return a token pair from `POST /register` | The backend settles it: `AuthController.register` answers 201 with a `UserResponse` and no tokens, so there is nothing to establish a session *from*. (a) is the tempting workaround and is worse than it looks — it turns one user action into two credential submissions, holds the password across both, and makes a **successful** registration report failure whenever the second call is the one that fails, which is the case where the user least understands what happened (their account exists; the screen says it does not). (b) is a backend change outside this milestone's boundary and one with its own question — whether an unverified account should hold a session — that belongs with email verification, not with a portal screen. The cost is one extra step, and it is paid back: the user learns their credentials work before they depend on them, and the flow the milestone's own §6.1 draws is exactly `/signup → /login → /onboarding → /dashboard` |
| D202 | (M23.2a) Onboarding **discards** the four API-key secrets `POST /api/v1/merchants` returns, and the consequence is recorded rather than hidden | (a) Display all four once on an onboarding success step; (b) store them server-side so a later screen can reveal them | The secrets are revealed exactly once — `ApiKeyService` stores a SHA-256 hash — so discarding them means a new merchant's usable secret comes from **rotating** a key on the API-keys screen rather than from signup. That is a real cost and it is the right one to pay here. (a) is the once-only reveal, which §5/M23 task 5 singles out as "the highest-consequence UI in the product" and which needs a copy-and-acknowledge step, an explicit warning, and its own tests; bolting it onto an account-creation form is precisely how that screen gets built carelessly, and it would put four live secret keys on a page a user reaches while still deciding whether to use the platform. (b) is D114's prohibition — the browser never holds a secret key — moved one hop and made worse, since it turns the portal into a store of every merchant's live credentials. Recorded because "we did not show them" and "we forgot to show them" look identical in a diff |
| D203 | (M23.2a) The browser suite's refresh-race assertion moves from **counting rotations** to **counting rejected rotations**; sidebar prefetching is kept | (a) Set `prefetch={false}` on the navigation links so the old count still holds; (b) raise the tolerated rotation count; (c) skip the refresh in middleware for prefetch requests | Turning on the first reachable nav destination (`/dashboard`) made Next.js prefetch it from every authenticated page, and a prefetch is an ordinary request that passes through D198's single refresh point and may legitimately rotate — so `rotations <= 1` began failing at three or four with the coordinator working perfectly. It had stopped measuring coordination and started measuring how many links are on the page. The replacement was chosen by **measurement**: with the mutex deliberately removed, ten navigations produced eighteen rotations, nine 401s and nine users signed out; with it, rejections stay at zero however much traffic the navigation graph generates. A rejection *is* the failure D197 exists to prevent, so the suite now asserts it directly. (a) disables a real performance feature to protect a test. (b) picks a number that means nothing and drifts with the next nav item. (c) changes the single refresh point — the one piece of M23.2 least safe to alter for a test's convenience — and would leave a prefetched render holding an expired token, so its cached payload could be the 401 the user sees on arrival |
| D204 | (M23.2b) The CSRF origin check accepts **sibling loopback spellings of its own address**, gated on `PORTAL_PUBLIC_ORIGIN` itself being a loopback origin, plus an opt-in `PORTAL_ADDITIONAL_ORIGINS` list | (a) Leave it strict and tell people to configure the origin correctly; (b) compare `Origin` against the request's own `Host` instead of against configuration; (c) gate the allowance on `NODE_ENV` rather than on the configured origin | (a) is what the code did and it is what produced the bug report: the portal answers on `localhost`, on `127.0.0.1` and on the machine's LAN address, only one of those can be in the variable, and a browser will offer the others unprompted. The failure was total — every form, including the one the whole entry flow depends on. (b) is what several frameworks do and it trusts a header a proxy sets, which turns a misconfigured proxy into a CSRF hole; the point of comparing against configuration is that configuration is not attacker-influenced. (c) was the first attempt and is subtly wrong: `docker compose up` runs the **production** bundle on `http://localhost:3000`, which is the same local-access situation, so an `NODE_ENV` gate would have left the commonest local setup broken. Gating on the configured origin asks the question that actually matters — is this portal addressed as a machine or as a site. The allowance stays narrow: `Origin` must equal the request's own `Host`, be a loopback name, and be plain `http:`; the scheme condition was added because a test caught `https://localhost:3000` passing, since `Host` carries no scheme and the two are different origins |
| D205 | (M23.2b) A cross-origin refusal and a stale CSRF token are reported as **different messages**, and a refused origin is logged with what arrived against what is accepted | (a) Keep one message, on the argument that neither should tell an attacker anything; (b) one message plus a server-side log | The two are different events with different fixes, and collapsing them cost a real bug report: a configuration mistake presented as *"this form expired before it was submitted"* on a form rendered one second earlier, which sent the investigation at signup and at CSRF, neither of which was involved. (a) mistakes secrecy for safety — whoever forged the request already knows their own origin, and whoever holds a stale token already knows it is stale, so the shared message protected nothing and hid everything. (b) is most of the answer and still leaves the person in front of the screen reading a sentence that is false. The messages are now specific, the log names the expected origins and the variable to change, and `guardFormRequest` applies both checks in one place so a new form cannot mislabel them |
| D206 | (M23.2b) Password recovery is built **entirely against the existing M15 contract**; the portal invents no endpoint, and `/reset-password` is the route identity-service already emails | (a) Add a portal-owned endpoint pair; (b) defer recovery until a later milestone | Nothing needed inventing, which the audit established before any code was written: `PasswordResetService` has shipped since M15 with an opaque token stored SHA-256-hashed, a 1-hour TTL, single use via `consume()`, and revocation of every refresh token on success — and `IdentityEventPublisher` has been building links as `{baseUrl}/reset-password?token=…` against a `base-url` whose own comment says it points at a frontend that does not exist yet. So the path and the parameter name were fixed by the backend eight milestones ago and this is the other end of them. (a) would duplicate a working flow behind a second contract. (b) leaves a sign-in page with no recovery and a backend quietly emailing links to a 404, which is worse than not having the feature. The portal's own additions are confined to what a *browser* client owes: a per-address request throttle whose throttled answer is byte-identical to its ordinary one, so the limiter cannot become the enumeration oracle `requestReset` deliberately refuses to be |
| D207 | (M23.3) A client component reads the platform through **one route that resolves GET operations only**, derived from the generated descriptors | (a) One route handler per surface, added by the milestone that owns it; (b) a general proxy that forwards any operation; (c) Server Actions for reads as well as mutations | §6.6 asks for a client query library with cursor pagination and refetch-after-mutation, and D187 forbids the browser calling the gateway, so *something* server-side must be callable after hydration. (a) is the conservative answer and means M23.6, M23.7 and M24 each write the same handler again — the duplication §15's rule-of-three exists to stop — while leaving pagination unbuildable until the first list screen. (b) is the same route without the constraint that makes it safe: filtering on the descriptor's own `method` is what keeps capture, refund and void behind Server Actions where M23.2's CSRF token and D190's confirmations live, and it is asserted from the contract rather than from a list so a mutation added later cannot slip in. (c) works and makes every read a POST carrying a CSRF token, which defeats HTTP caching semantics, cannot be opened in a tab while debugging, and gives the query library nothing to key on. The route never accepts a merchant or a mode from the client: both come from the sealed session, and `?mode=live` fails as an undocumented parameter before a request is built |
| D208 | (M23.3) The command palette's object lookup classifies a pasted id **by the contract's own id shapes**, and a UUID is tried as a payment and then as a refund | (a) Follow the M23.1 note and switch on a `pay_` prefix; (b) ask the user which kind of object it is; (c) fan out to all three endpoints in parallel | (a) describes an API this platform does not have. `PaymentResponse.id` and `RefundResponse.id` are `format: uuid`; only `EventResponse.id` carries a prefix (`evt_` plus 32 hex). The note predates the contract, and following it would have shipped a lookup that never matched anything a merchant could paste. (b) asks the user for something the platform can determine. (c) spends three requests on every paste and races two 404s against one answer to save a single sequential round trip in the uncommon case. Payment first, because that is overwhelmingly what is pasted; a 404 means "not this kind" and moves on, while a 401 or 403 stops the search rather than being reported as "no such object" |
| D209 | (M23.3) `useQueryScope` **throws** when there is no merchant, and the key factory is the only way to build a key | (a) Default the scope to a placeholder when it is absent; (b) let callers assemble keys and document the rule | The rule §6.6 states — mode in every query key — is exactly the kind that holds for a year and then does not, because it is remembered rather than enforced. Making the scope the first argument of the only exported key builder turns a convention into the type system's problem. (a) is the specific failure that makes it worthless: a placeholder scope produces keys that *collide across merchants*, the one thing the scope exists to prevent, and does it silently. (b) is where this started, and is how the second draft of a data layer acquires three spellings of one key. `useOptionalQueryScope` exists for the two components that legitimately render before a merchant does |
| D210 | (M23.4) A settings save confirms itself by **navigating the document** to `/settings?saved=…`, and the action returns only which section saved | (a) `revalidatePath` from the action; (b) `router.refresh()`; (c) `redirect()` from the action; (d) keep the confirmation in `useActionState` alone | Each of the first three was built, measured against the real stack, and failed in a way the type checker and the unit suite both called correct. (a) refreshed the shell and re-rendered the subtree holding the form, discarding `useActionState` — the value persisted every time while the screen intermittently said nothing. (b) kept the confirmation and did not reliably re-render the shared `(app)` layout, so the header went on showing the old business name after a rename that had definitely succeeded. (c) does not navigate at all here: the target differs from the current URL only in its query string, so the router treats it as the page it is already on — the save persisted, the URL never changed, nothing moved. (d) shows a confirmation over stale chrome, which is the same defect as (b) with no way to notice it. A document load re-renders layout *and* page on the server and makes the confirmation a flag the page owns rather than client state something can discard; the cost is one navigation on a rare, deliberate action, which is what a plain HTML form post would have cost anyway. The flag is a flag, never a message: the page renders fixed copy from it and echoes nothing out of the URL, the rule `/login` already follows for `?registered=1` |
| D211 | (M23.4) `/settings` **names** the two sections it cannot build and says which milestone owns them | (a) Omit them until the backend supports them; (b) build them against invented endpoints; (c) disable the controls with a tooltip | §6.1 puts profile, merchant, team and an API version pin on this route, and the platform answers two of the four. Team membership is not deferred by preference — no team, membership or invitation entity exists anywhere, so there is nothing to call. `pinned_api_version` is a real column but rides only on the internal `ApiKeyVerifyResponse`; putting it on `MerchantResponse` is a contract change M23.4 does not authorise. (a) makes a finished screen look unfinished, which is the reading a settings page with a missing section always gets. (b) is the one thing this milestone forbids outright. (c) implies the feature exists and is switched off. The account details are read-only for the same reason and say so: `UserController` exposes no update endpoint, and the password control is honest that it emails a link, because identity-service has no authenticated change-password path — only M23.2b’s reset flow |
| D212 | (M23.5) Revocation’s typed-name confirmation is checked against the **stored** name, re-read from merchant-service inside the action | (a) Carry the expected name in a hidden field and compare the two halves of one submission; (b) rely on the dialog’s disabled button | §6.3 asks for typing the key name as "a deliberate friction on a destructive, irreversible action", and (a) is not one: both sides of the comparison come from the same request, so it confirms only that the client agrees with itself and is satisfied by two matching blanks from anything that skipped the dialog. (b) is a suggestion to anything that is not a browser — a Server Action is an HTTP endpoint. Re-reading costs one extra call on the rarest and most destructive action on the screen, and buys two things: the friction becomes a property of the request, and a key that was already revoked or never belonged to this merchant is reported absent instead of revoked twice. The comparison is trimmed but case-sensitive, because "Prod" and "prod" are two different keys and the point is to make someone read what they typed |
| D213 | (M23.5) `ApiKeyResponse` gains **`graceExpiresAt`** — the one backend change M23.5 required | (a) Explain the grace window only in the rotation dialog, before the fact; (b) record it as debt and ship rotation without any grace indication | `ApiKey` has stored the field since M15 and `isActive` decides on it, but the DTO never carried it — so after a rotation the retired key came back with `revokedAt` and `expiresAt` both null, **identical on the wire to the replacement that had just superseded it**. Two rows, same name, no way to say which one dies or when. (a) puts the explanation at the only moment it cannot be acted on and leaves the list permanently wrong afterwards; (b) ships the weakest possible reading of a requirement §6.3 states outright. The change is additive, on a DTO no published contract describes: `PublicApiDocumentContract` fails the build if any documented path starts with `/api/v1`, so `docs/openapi.yaml`, both SDKs and the two generated trees are provably unaffected — confirmed by running all three OpenAPI gates and the parity diff after the change (0 breaking, 0 additive, baseline in sync). The roadmap’s own note claimed this field was already visible; §14 now records that it was not |
| D214 | (M23.5) A Server Action that returns a **once-only secret** must not change anything that re-renders the page it returns to | (a) Reseal the session to the new key’s mode inside `createKeyAction`, so the list shows what was just made; (b) revalidate the route after issuing | This is D210’s lesson arriving by a second route, and it cost more here. §6.3 puts a mode selector on creation while the list is scoped to the session’s mode, so (a) is the obvious fix — and measured against the real stack it destroyed the secret every time: the reseal succeeded, merchant-service stored the key, the action answered 200 and the page re-rendered to completion under the new mode, and the reveal never appeared. Setting a cookie makes Next re-render the route inside the action’s own response, and that render lands on the tree holding the `useActionState` waiting for the return value; under a changed mode it is not the same tree, so the state waiting for the result does not survive to receive it. The form sat on "Creating…" while a key existed whose only readable copy was already gone. The mode change therefore happens **after** the secret is copied: the reveal’s acknowledge button submits `POST /api/session/mode`, which is where mode has been changed since M23.2 (D184) and which redirects back as a fresh document. Nothing re-renders while a secret is on screen |
| D215 | (M23.6) A saved view **is its URL** | (a) `localStorage` presets, closest to §6.2’s "persisted per user"; (b) a preferences entity in identity-service or merchant-service; (c) both, with the URL carrying filters and local storage naming them | §6.2 asks for saved views and nothing in the platform can persist a user preference — no preferences entity exists in any service, and M23.6 is not authorised to invent one, which rules out (b) as a scope expansion into someone else’s milestone. (a) is per **browser**, not per user: the view a developer saved is invisible on their laptop, which is a smaller promise than §6.2’s wording while looking like a larger one. It also adds client-side state to a screen that otherwise needs none. A URL is shareable, bookmarkable, reloadable, back-button-correct and diffable in a ticket — a developer investigating a spike pastes the link and their colleague sees the same rows, which is the thing a saved view is actually *for*. (c) was rejected as two mechanisms for one concept. The URL speaks the contract’s own vocabulary (`amount_min`, `created_after`) so a link is self-describing, and it carries **no merchant and no mode**: both come from the sealed session, and `mode` is not a parameter of any operation, so a shared link means "these filters, my data" and cannot mean anything else |
| D216 | (M23.6) CSV export writes **the rows that are loaded**, and the button says how many | (a) Page to the end of the result set on click, then write the file; (b) a server-side export route that streams the whole set; (c) no export until an endpoint exists | §6.2 asks for bulk export and the contract has no export operation. (a) is the one that looks right and behaves worst: on a merchant with a year of payments it is hundreds of sequential requests against their own rate limit, started by a button that looks instant, with no progress and no way to stop — and the first thing it does is spend the quota of the account asking for it. (b) moves the same problem server-side and adds a second path over `listPayments` that must agree with the first about filters and cursors forever. (c) drops a stated requirement. Naming the count on the button — "Export 250 rows" — makes the scope of the file the user’s own decision, since loading more rows is already an explicit act. Amounts are exported in **minor units**, as the platform gives them: a spreadsheet is exactly where a divided-by-100 float becomes a reconciliation dispute. Values beginning `=`, `+`, `-` or `@` are prefixed, because metadata is arbitrary merchant-supplied text and CSV injection is a path from a merchant’s own customer into a formula on someone’s machine |
| D217 | (M23.6) The transport and the read route accept the **`deepObject` spelling** of a declared map parameter | (a) Send `metadata=…` and hope; (b) let arbitrary query parameters through; (c) drop metadata filtering and record it as debt | Descriptors carry parameter *names*, and `metadata` is `style: deepObject` — the request the contract documents is `?metadata[order_id]=abc`, which is not the name in the list. Both layers rejected it, so §6.2’s metadata filter could not be sent at all. (a) is the exact failure the parameter check exists to prevent: the platform ignores an unrecognised filter and answers with a correct-looking **unfiltered** page, which on a payments screen means showing a merchant rows they did not ask for and believing the filter worked. (b) throws away the check that turns a typo into an exception. (c) narrows a requirement rather than meeting it. The rule is still the descriptor’s: a bracket suffix is accepted **only** when the base name is declared, so `metadata[x]` passes for an operation documenting `metadata` while `mode[x]`, `merchant[id]` and `stat[us]` fail exactly as before — asserted in both directions |
...
- **Rotate-with-grace has no proactive "your grace window is about to lapse" notification** — no email, no webhook. A developer must look. *(Corrected 2026-08-09: this entry previously claimed `graceExpiresAt` was already visible on `GET /api/v1/merchants/me/api-keys`. It was not — `ApiKeyResponse` did not carry the field, so a rotated-out key was indistinguishable from a healthy one, and the dashboard could not have shown a deadline even if a developer did look. M23.5 added the field (D213); what remains open is only the notification.)* No milestone currently owns the notification.
