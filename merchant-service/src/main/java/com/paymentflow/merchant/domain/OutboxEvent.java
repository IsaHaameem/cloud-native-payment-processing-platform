...
/**
 * Transactional outbox row (D3, mirrored from payment-service's M5 pattern): written
 * in the same database transaction as the merchant/key mutation it records, so the
 * state change and the fact that an event needs publishing either both commit or
 * neither does. {@link com.paymentflow.merchant.outbox.OutboxRelay} polls for
